...
#ifndef ACCURACY_EVALUATION_CPP_EVALUATION_H
#define ACCURACY_EVALUATION_CPP_EVALUATION_H
...
class Evaluation {
 public:
  ...
  Evaluation(double avgAccuray, double errRate, double precisionMicro,
             double recallMicro, double fscoreMicro, double precisionMacro,
             double recallMacro, double fscoreMacro)
  ...
  void evaluation(Confusion& confusion) {
    ...
    // Precision-Macro (An average per-class agreement of the data class labels
    // with those of a classifiers)
    double precisionMacro = 0.0;

    // Recall-Micro (An average per-class effectiveness of a classifier to
    ...
    // Fscore-Macro (Relations between data’s positive labels and those given by
    // a classifier based on a per-class average)
    double fscoreMacro = (((beta * beta) + 1) * precisionMacro * recallMacro) /
    ...
    // Fscore-Micro (Relations between data’s positive labels and those given by
    // a classifier based on sums of per-text decisions)
    double fscoreMicro = (((beta * beta) + 1) * precisionMicro * recallMicro) /
...
#endif  // ACCURACY_EVALUATION_CPP_EVALUATION_H
