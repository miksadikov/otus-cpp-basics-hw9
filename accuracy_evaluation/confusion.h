...
#ifndef ACCURACY_EVALUATION_CPP_CONFUSION_H
#define ACCURACY_EVALUATION_CPP_CONFUSION_H
...
class Confusion {
 public:
  ...
  Confusion(int classes, int samples)
      : _classes(classes),
        _samples(samples),
        _per(classes, vector<double>(4)),
        _ind(classes, vector<string>(classes)),
  ...
  void convertToBooleanMatrix(vector<int> targets, vector<int> outputs,
                              vector<vector<double>>& tar,
                              vector<vector<double>>& out) {
    int numClasses = *max_element(targets.begin(), targets.end()) -
                     *min_element(targets.begin(), targets.end()) + 1;
  ...
  void confusion(vector<vector<double>> targets,
    ...
    /* confusion takes an SxQ (S:Classes; Q:Samples)target and output matrices
       ...
       the largest Y indicating the models output class.
        ...
        associated with the ith class:
            S(i,1) = false negative rate = false negatives / all output
       negatives S(i,2) = false positive rate = false positives / all output
       positives S(i,3) = true positive rate = true positives / all output
       positives S(i,4) = true negative rate = true negatives / all output
       negatives
           */
    ...
    _classes = numClasses;
    _samples = numSamples;
    // Transform outputs   (maximum value is set to 1 and other values to 0,
    // column-wise)
    ...
    for (int col = 0; col < numSamples; col++) {
      if (ind[i[col]][j[col]] == "")
        ...
        ind[i[col]][j[col]] =
            ind[i[col]][j[col]].append(",").append(to_string(col));
    }
...
#endif  // ACCURACY_EVALUATION_CPP_CONFUSION_H
